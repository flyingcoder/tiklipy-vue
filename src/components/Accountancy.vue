<script setup>
    ...
    const getData = () => {
        backEndModel.getAccountancy().then((data) => {
            accountancy.value = data.data.accountancy;
...
<template>
    <div class="flex flex-wrap text-black mt-16 justify-between px-3 ">
        <router-link v-for="(data, index) in accountancy" :key="index + '-accountancy'" class="flex w-full md:w-[48%] lg:w-[32%] xl:max-w-[30rem] justify-center " :to="{ name: 'helps', params: { category: data.category, slug: data.slug } }">
            <the-card href="#" class="!max-w-2xl bg-white rounded-lg mb-5 flex border-none rounded-lg shadow-none hover:bg-white hover:shadow-md bg-[url('/p-1.png')] bg-no-repeat bg-contain" style="">
                <div class="p-4  dark:bg-gray-800 dark:border-gray-700">
                    <img :src="sitename.logoSrcIcon" class="mr-5 mb-7" width="70" alt="">

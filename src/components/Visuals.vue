<script setup>
    ...
    const getData = () => {
        backEndModel.getVisuals().then((data) => {
            visuals.value = data.data.visuals;
        }).catch((error) => {
...
<template>
    <div class="flex flex-wrap text-black mt-16 justify-between px-3 ">
        <router-link v-for="(data, index) in visuals" :key="index + '-visuals'" class="flex w-full md:w-[48%] lg:w-[32%] xl:max-w-[30rem] justify-center " :to="{ name: 'helps', params: { slug: data.slug } }">

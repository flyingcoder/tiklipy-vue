<script setup>
    ...
    const getData = () => {
        backEndModel.getApplications().then((data) => {
...
<template>
    <div class="flex flex-wrap text-black mt-16 justify-between px-3 ">
        <router-link v-for="(data, index) in applications" :key="index + '-applications'" class="flex w-full md:w-[48%] lg:w-[32%] xl:max-w-[30rem] justify-center " :to="{ name: 'helps', params: { category: data.category, slug: data.slug } }">

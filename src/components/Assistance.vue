<script setup>
    ...
    const getData = () => {
        backEndModel.getAssistance().then((data) => {
            assistance.value = data.data.assistance;
        }).catch((error) => {
...
<template>
    <div class="flex flex-wrap text-black mt-16 justify-between px-3 ">
        <router-link v-for="(data, index) in assistance" :key="index + '-assistance'" class="flex w-full md:w-[48%] lg:w-[32%] xl:max-w-[30rem] justify-center " :to="{ name: 'helps', params: { slug: data.slug } }">
